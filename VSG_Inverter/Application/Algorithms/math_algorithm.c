...
--------------------------------------------------------------------------------------------------*/
inline fp32 my_clip(fp32 fpValue, fp32 fpMin, fp32 fpMax)
{
	if (fpValue <= fpMin)
	{
		return fpMin;
...
inline uint32_t Clip_Uint(uint32_t value, uint32_t fpMin, uint32_t fpMax)
{
	if (value <= fpMin)
	{
		return fpMin;
...
********************************************************************/
int temp;
int32_t Round(fp32 fp_Round_Number)
{
	//    if (fp_Round_Number >= 0)
	//    {
	//		return (int16_t)(fp_Round_Number + 0.5f);
	//    }
	//    else
	//    {
	//		return (int16_t)(fp_Round_Number - 0.5f);
	//    }
	fp32 xiaoshu, zhengshu;
	xiaoshu = ((int)(fp_Round_Number * 10)) % 10;
	zhengshu = (int)(fp_Round_Number * 10) / 10;
	if (xiaoshu > 5)
	{
		fp_Round_Number = (fp32)(zhengshu + 1);
		temp = fp_Round_Number;
		return fp_Round_Number;
	}
	else
	{
		temp = zhengshu;
		return (int32_t)zhengshu;
	}
}
...
//fp32 SineSignal_Output(fp32 A, fp32 f, fp32 acc_tim, fp32 dt)
//{
//	static fp32 A_in = 0;
//	static uint16_t tick = 0;
//	fp32 sinx, cosx;
//	RampSignal(&A_in, A, A * dt / acc_tim);
//	arm_sin_cos_f32(360 * f * tick * dt, &sinx, &cosx);
//	tick++;
//	if (tick == 1.0f / dt)
//		tick = 0;
//	return A_in * sinx;
...
fp32 Linear_LeastSquare(fp32 *px, fp32 *py, unsigned char order)
{
	unsigned char i;
	fp32 x_y_ave = 0, x_ave = 0, y_ave = 0, x_sqr_ave = 0;
	for (i = 0; i < order; i++)
	{
		x_y_ave += (*(px + i)) * (*(py + i));
		x_ave += *(px + i);
	...
	x_ave /= (fp32)order;
	y_ave /= (fp32)order;
	x_sqr_ave /= (fp32)order;
	return (x_y_ave - x_ave * y_ave) / (x_sqr_ave - x_ave * x_ave);
}


const fp32 fast_atan_table[257] =
	{
...
fp32 fast_atan2(fp32 y, fp32 x)
{
	fp32 x_abs, y_abs, z;
	fp32 alpha, angle, base_angle;
	int index;
